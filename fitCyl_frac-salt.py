...
ios=2*200*pow(10,-3)*pow((1.6022*pow(10,-19)),2.)*6.023*pow(10,23)*pow(10,3) #130/65 ionic strength of pH7 water, concentration is in #ions/m^3, hence avagadro's number
Ld=pow((e0*ew*kbt/ios),0.5) #pow((8*np.pi*6.023*pow(10,23)*pow(1.6022,2)*pow(10,-38)/(ew*e0*kbt)),-0.5) #debye length using Mezzenga's expression, in meters
...
def PofQ_PRISM (r,La,mu,v,q):  #The P(Q) from interacting rigid rods using a corrhole and P(Q) for long cylinders
    ...
    PofQ= v*pCyl(r,La,q)#np.divide(pCyl(r,La,q),(1+np.multiply(corr(mu,v,La,q),pthinCyl(La,q))))
    return PofQ 
...
def IofQ_Cyl_corrhole_frac (q,*params): # For fractal forming straight cylinders
    ...
    final_func=I0*np.multiply(ff(r,La,mu,v,q), (1.+np.multiply(betaf(r,La,q),sf(q,La,e_cluster,df)))) + bkg
    return final_func
...
eclust=2*np.pi/Q[0]*1000 #Ang
#fracdim=2. #dimensionless
cd= 0.22*pow(10,-21) #0.065*1.6022*pow(10,-19) #charge density in ~5e/A

guessval0=np.array([nD,rad,Lapp,cd,eclust])
guesslow0=np.array([0.01*nD,8,80,cd*0.99,10000])
guesshigh0=np.array([100*nD,12,Lapp*10,1.01*cd,pow(10,5)*eclust])
gscale0=[pow(10,21),0.1,0.1,pow(10,-100),100.]
fit_params0, unc_params0=Iqfitting(q=Q,data=IQ, data_err=IQ_ERR, 
                                 guessval=guessval0, guesslow=guesslow0,guesshigh=guesshigh0)


b,c,d,e,f=fit_params0 #scale,nD,rad,Lapp,cd,eclust,fracdim                                                                  
...
b,c,d,e,f=fit_params1 #scale,nD,rad,Lapp,cd,eclust,fracdim
...
I0fitted=(dsldsq*((np.pi*pow(c,2)*d)**2.))*pow(10,-22)*b
        # Writing to file
filename="Fit_f-"+sys.argv[1]
f=open(filename,'w')
f.write("\n Given params: fracdim = %.2e, \t"%fracdim)
f.write("\n Bkg value= %.3e, \t"%bkg)
f.write("\n Given guess values: %.2e,%.2e,%.2e,%.2e,%.2e,%2e"%(scale,nD,rad,Lapp,cd,eclust))
f.write("\n Fitted params scale,number density v(#/m3), radius r(A),\napparent length Lapp(A),charge density cd(C/A), cluster size eclust(A)\n")
